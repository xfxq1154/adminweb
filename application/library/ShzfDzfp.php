...
class ShzfDzfp {
    ...
    public function fpkj($order, $detail) {
        ...
        $kjxx_data = array(
            ...
            'GMF_MC'        => $order['title'],         //*发票抬头
            'GMF_DZDH'      => '',                      //购买方地址
            'GMF_YHZH'      => '',                      //购买方银行账号
            'KPR'           => $order['kpr'],           //*开票人
            'SKR'           => isset($order['payee']) ? $order['payee'] : '',          //收款人
            'FHR'           => isset($order['review']) ? $order['review'] : '',        //复核人
            'YFP_DM'        => $order['type'] == 1 ? $order['invoice_code'] : '',    //原发票代码 红字发票时必须
            'YFP_HM'        => $order['type'] == 1 ? $order['invoice_number'] : '',      //原发票号码 红字发票时必须
            'JSHJ'          => $order['type'] == 1 ? -$order['jshj'] : $order['jshj'], //*价税合计 单位元 (2位小数)
    ...
    public function batch($data, $type){
        ...
        foreach ($data as $key => $value){
            ...
            $kj_data[$key]['DW']    = ''; //规格型号
            $kj_data[$key]['XMSL']  = $type == 1 ? -$value['num'] : $value['num'];      //项目数量
            $kj_data[$key]['XMDJ']  = $value['price'];                                  //项目单价 小数点后六位 不含税
