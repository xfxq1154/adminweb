...
use Trait_Redis;
    ...
    public $tableName = '`a_audio_content_record`';
    public $adminLog;
    ...
    public function getAudioList($aid, $page = 1, $size = 20) {
        ...
        try {
            $sql = "SELECT * FROM " . $this->tableName . "  where r_aid='{$aid}' and r_content !=''  ORDER BY r_id desc  limit " . $limit;
            ...
            if ($rs) {
                foreach ($rs as $key => $val) {
                    $return[$key] = CustomArray::removekeyPrefix($val, 'r_');
                }
            }
    ...
    public function findById($id) {
        ...
        try {
            $sql = 'SELECT * FROM ' . $this->tableName . ' WHERE r_id = :id LIMIT 1';
            $stmt = $this->dbMaster->prepare($sql);
            $stmt->execute([':id' => $id]);

            $rs = $stmt->fetch(PDO::FETCH_ASSOC);

            return $rs ? CustomArray::removekeyPrefix($rs, 'r_') : false;
        } catch (PDOException $e) {
            die($e->getMessage());
        }
    }
    ...
    public function findCountByAid($aid) {
        ...
        try {
            $sql = 'SELECT count(r_id) as num FROM ' . $this->tableName . ' WHERE r_aid = :id  LIMIT 1';
            $stmt = $this->dbMaster->prepare($sql);
            $stmt->execute([':id' => $aid]);
